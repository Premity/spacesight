...
export default function PlanetCard({ planet }) {
  const getConfidenceColor = (conf) => {
    switch (conf.toLowerCase()) {
      case "high":
        return "text-green-400 bg-green-400/10 border-green-400/30 shadow-[0_0_15px_rgba(74,222,128,0.2)]";
      case "medium":
        return "text-yellow-400 bg-yellow-400/10 border-yellow-400/30 shadow-[0_0_15px_rgba(250,204,21,0.2)]";
      case "low":
        return "text-red-400 bg-red-400/10 border-red-400/30 shadow-[0_0_15px_rgba(248,113,113,0.2)]";
      default:
  ...
  return (
    <div className="group bg-space-surface/60 backdrop-blur-md border border-space-purple/30 rounded-3xl p-10 shadow-[0_0_20px_rgba(124,58,237,0.15)] hover:shadow-[0_0_40px_rgba(124,58,237,0.3)] hover:-translate-y-2 transition-all duration-500 relative overflow-hidden flex-1 min-w-[300px]">
      ...
      <div className="flex justify-between items-start mb-10 relative z-10">
        ...
        <div
          className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest border ${getConfidenceColor(planet.confidence)}`}
        >
          {planet.confidence}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-y-8 gap-x-6 relative z-10">
        <div>
          ...
          <span className="font-mono text-xl text-white">
            {planet.orbitalPeriod ? planet.orbitalPeriod.toFixed(2) : "—"}{" "}
            <span className="text-xs text-space-text/50 font-sans tracking-wide">days</span>
          </span>
        </div>
        <div>
          <span className="text-space-text/40 text-[0.65rem] font-bold uppercase tracking-widest block mb-2">
          ...
          <span className="font-mono text-xl text-white">
            {planet.transitDepth ? planet.transitDepth.toFixed(2) : "—"}
          </span>
        </div>
        <div className="col-span-2">
          ...
          <span className="font-mono text-xl text-white">
            {Number(planet.estimatedRadius).toFixed(2)}{" "}
            <span className="text-xs text-space-text/50 font-sans tracking-wide">Earth Radii</span>
          </span>
        </div>
      </div>
    </div>
