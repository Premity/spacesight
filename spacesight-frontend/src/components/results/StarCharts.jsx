...
export default function StarCharts({ star }) {
  ...
  const fluxDomain = [Math.max(0, minFlux - fluxPadding), maxFlux + fluxPadding];
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 w-full animate-fade-in">
      ...
      <DetailChartCard
        title="Orbital Diagram Model"
        tooltip="A schematic view of detected planets orbiting their host star. Orbit sizes are scaled relative to each other by orbital period — longer period = larger orbit. Hover over a planet dot for details."
      >
        <div className="w-full h-full flex flex-col justify-center items-center relative overflow-hidden">
          ...
          {star.planets.length > 0 ? (
            ...
            <span className="text-space-text/40 font-mono relative text-sm uppercase tracking-widest bg-space-surface/80 px-4 py-2 rounded-full border border-white/5 z-10">
              Stellar Target Only
            </span>
          )}
        </div>
      ...
      <DetailChartCard
        title="Planet Size Reference vs Local System"
        tooltip="Visual comparison of detected planet radii against Earth and Jupiter for scale. Sizes are proportional — a planet at 11.2 R⊕ would be Jupiter-sized."
      >
        <div
          className="w-full h-full flex items-end justify-center pb-8 gap-10 px-6 overflow-x-auto"
          style={{ scrollbarWidth: "none" }}
        >
          {/* Earth reference — scaled to match Jupiter anchor (128px = 11.2 R⊕) */}
          <div className="flex flex-col items-center gap-3 shrink-0">
            <div
              style={{ width: Math.round(128 / 11.2), height: Math.round(128 / 11.2) }}
              className="rounded-full bg-blue-400/80 shadow-[inset_-3px_-3px_8px_rgba(0,0,0,0.5)]"
            ></div>
          ...
          <div className="flex flex-col items-center gap-3 shrink-0 ml-4 border-l border-white/10 pl-10">
            ...
            <span className="text-[10px] text-space-text/50 font-mono tracking-widest uppercase">
              Jupiter (11.2 R⊕)
            </span>
          </div>
        </div>
  ...
  return tip
    ? createPortal(
        ...
        >
          ...
          <p className="text-space-text/70 text-[10px] font-mono whitespace-nowrap">
            {Number(planet.estimatedRadius).toFixed(2)} R⊕
          </p>
