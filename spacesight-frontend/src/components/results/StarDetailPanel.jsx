...
export default function StarDetailPanel({ star }) {
  ...
  // The parent remounts this panel via key={selectedStar}, so isMounted starts false
  // for every star; the short delay staggers the charts' entrance.
  useEffect(() => {
    ...
    <div className="flex flex-col gap-10 w-full animate-fade-in relative z-10 pt-4 pb-12">
      {star.error ? (
        ...
        <div className="w-full bg-yellow-500/5 border border-yellow-500/30 rounded-3xl p-10 md:p-14 text-center shadow-[0_0_30px_rgba(234,179,8,0.1)] backdrop-blur-md flex flex-col items-center">
          ...
          <h3 className="text-yellow-400 font-orbitron font-bold text-2xl md:text-3xl mb-4 drop-shadow-sm">
            No Planetary Candidates Detected
          </h3>
          <p className="text-yellow-400/80 text-lg md:text-xl font-light max-w-3xl mx-auto leading-relaxed">
            We compute with <span className="font-bold text-yellow-400">{star.noPlanetConfidence}% confidence</span>{" "}
            that no major transits identifying exoplanetary bodies exist within the observation window for target{" "}
            <span className="font-mono bg-yellow-500/10 px-3 py-1 rounded inline-block text-yellow-300 ml-1 border border-yellow-500/20">
              {star.name}
            </span>
            .
          </p>
      ...
      <div className="mt-4 border-t border-white/10 pt-10">
        <h3 className="font-orbitron font-bold text-xl text-white mb-8 text-center md:text-left drop-shadow-md">
          Target Physical Analysis
        </h3>
        {isMounted ? (
