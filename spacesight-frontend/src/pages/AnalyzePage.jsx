...
export default function AnalyzePage() {
        ...
        <div className="w-full max-w-lg bg-space-surface/50 border border-white/10 p-10 rounded-[2rem] backdrop-blur-md shadow-[0_0_40px_rgba(0,0,0,0.5)] relative overflow-hidden">
          ...
          <div className="flex flex-col gap-6">
            {STAGES.map((stageName, idx) => {
              ...
              const isActive =
                pipelineState.stageIndex === stageIndex &&
                pipelineState.stageIndex > 0 &&
              ...
              return (
                <div
                  key={stageName}
                  className={`flex items-center gap-5 transition-all duration-300 ${isFuture ? "opacity-40" : "opacity-100"} ${isActive ? "scale-105 transform origin-left" : ""}`}
                >
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center border-2 shrink-0 transition-all duration-300 ${
                      isActive
                        ? "border-space-purple bg-space-purple/20 shadow-[0_0_15px_rgba(124,58,237,0.5)]"
                        : isCompleted
                          ? "border-space-teal bg-space-teal/20"
                          : "border-space-text/20 bg-space-surface"
                    }`}
                  >
                    {isActive && (
                      <svg
                        className="animate-spin h-5 w-5 text-space-purple"
                    ...
                    {isFuture && <span className="w-2 h-2 bg-space-text/30 rounded-full"></span>}
                  </div>
                  <div className="flex flex-col w-full">
                    <span
                      className={`font-orbitron tracking-widest uppercase text-sm ${isActive ? "text-space-purple font-bold drop-shadow-md" : isCompleted ? "text-space-teal" : "text-space-text/50"}`}
                    >
                      {stageName}
                    </span>
  ...
  return (
    <div className="min-h-[80vh] pt-32 pb-20 px-6 flex flex-col items-center justify-center relative z-10 w-full text-space-text font-inter">
      ...
      <p className="text-space-text/60 text-center max-w-lg mb-12">
        Submit Kepler light curve data (.npz or .zip) to initiate the transit detection pipeline.
      </p>
      ...
      >
        <input {...getInputProps()} />
        <svg
          className={`w-20 h-20 mb-6 transition-colors duration-300 ${isDragActive ? "text-space-purple" : "text-space-purple/60"}`}
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
