...
export default function ResultsPage() {
  ...
  const [clickedStar, setSelectedStar] = useState(null);

  // Derived, not synced via effect: single-star results always show their one star;
  // multi-star defaults to the first star until a row is clicked.
  const selectedStar =
    results && (results.type === "single" || !clickedStar) ? (results.stars[0]?.name ?? null) : clickedStar;
      ...
      <div className="max-w-7xl w-full flex flex-col gap-12">
        <div id="results-export-target" className="flex flex-col gap-12 w-full pt-4 rounded-xl">
          ...
          <section id="planet-table">
            <h2 className="font-orbitron text-2xl font-bold text-white mb-6 border-b border-white/10 pb-4">
              Planet Candidates Directory
            </h2>
          ...
          <section id="star-detail" ref={detailRef} className="pt-10 scroll-m-24">
                  ...
                  {[...results.stars]
                    ...
                    .map((s) => (
                      <option key={s.id} value={s.name}>
                        {s.name} ({s.planets.length} planets)
                      </option>
                    ))}
                </select>
              )}
            ...
            {/* key remounts the panel on star change so its entrance animation replays */}
            <StarDetailPanel key={selectedStar} star={results.stars.find((s) => s.name === selectedStar)} />
          </section>
        </div>
      </div>
    </div>
